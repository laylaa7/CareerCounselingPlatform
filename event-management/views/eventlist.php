	  ...
	  foreach($records as $rec) {
        ...
        <tr>
		  ...
		  <td><?php if($status == "PENDING") {?>
			...
			&nbsp;<a href="javascript:deleteUser('<?php echo $user_id ?>', '<?php echo $rid ?>');">Delete</a>
            <?php } else { ?>
			<a href="javascript:deleteUser('<?php echo $user_id ?>', '<?php echo $rid ?>');">Delete</a>
			<?php }//else ?>
          </td>
...
function deleteUser(userId, rid) {
	if(confirm('Deleting user will also delete it\'s booking from calendar.\n\nAre you sure you want to priceed ?')) {
		window.location.href = '<?php echo WEB_ROOT; ?>api/process.php?cmd=delete&userId='+userId + '&rid='+rid;
	}
}
