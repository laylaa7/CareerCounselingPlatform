...
function bookCalendar() {
	...
	*/
	...
	$sql = "INSERT INTO tbl_reservations (uid, NameOfStudent, rdate, status, comments, bdate) 
			VALUES ($userId, '$NameOfStudent', '$bkdate', 'PENDING', '', NOW())";
	dbQuery($sql);
...
function regDelete() {
	$userId	= $_GET['userId'];
	$rid = $_GET['rid'];
	$sql1	= "DELETE FROM tbl_reservations WHERE uid = $userId AND id=$rid";
	dbQuery($sql1);
...
function calendarView() {
	...
	$bookings = array();
	$sql	= "SELECT u.name AS u_name, u.id AS user_id, r.rdate, r.status , r.id as rid
	...
	while($row = dbFetchAssoc($result)) {
		...
		$book->url = WEB_ROOT . 'views/?v=USER&ID='.$user_id."&RID=".$rid;
		$bookings[] = $book; 
	}
